...
def adventure_method player, goblin #calls the variables
    ...
    adventure = true
    while (adventure)
        #We have a loop based around adventuring with a loop based around fighting inside of it
        puts "you have encountered a Goblin! \nWhat will you do? \na-fight em \nb-flee"
        #when encountering the monster the player is given the opportunity to flee
        option =gets.chomp.downcase
        if option == "a"
        #do the fight
        puts "thus the fight begins, you have #{player["hp"]} HP"
            #When the fight begins the we display the player's current HP to them
            fight = true
            while (fight)
                ...
                player["hp"] -= goblin["atk"]
                 #the player and the goblin trade blows based on their respective "atk" stat lowering their respective hp until one equals zero

                    if player["hp"] <= 0
                        puts "you've been slain by the goblin"
                        adventure = false
                        #the player loses and the larger adventure loop ends 
                    elsif goblin ["hp"] <= 0
                        puts "You have slain the beast!" 
                        puts "your currently have #{player ["hp"]} HP."
                        #the player wins (rewards for winning haven't been implemented yet)
                        #players current HP is displayed to them after the fight
                       
                        puts "Would you like to continue adventuring? y/n"
                            fightOption = gets.chomp.downcase
                            #player is given the option to continue adventuring or return to town for healing or shopping after each fight
                        
                        if fightOption == "y"
                            puts "Good on ya adventurer \nYou take a deep breath and continue on your way"
                            #choosing to keep adventuring, returns the player to the fight top of the fight loop to continue battling goblins
                            #====NOTE====
                            #this is currently glitched, once beating one goblin the monsters health does not reset, so the player defeats the next goblin in a single hit despite its declared 3 HP
                            #Solutions ----> either do a +3 to goblin hp when the player chooses to continue adventuring/state the goblins HP at the top of the fight loop so it resets every time the fight begins
                            #ALT IDEA ----> randomly raise the goblins HP from 1 - 3, to dynamically change the amount of health it starts the continued fights with --> would provide some flavor to fights and not have them repeat 3 blows back and forth indefinitely
                        elsif fightOption == "n"
                            puts "*you return to town*"
                            fight = false
                            adventure = false
                            #you return to town ending both the fight AND adventure loop
    ...
    if option =="b"
        ...
        adventure = false
        #you flee from battle ending the adventure loop without entering the fight loop
    end
end
...
shop = true
    while shop #begin while loop

puts "Welcome to the shop! You have #{player["gold"]} gold"
puts "a: buy an item."
puts "b: sell an item."
puts "c: exit shop."
shop_choice = gets.chomp.downcase #shop choice sends player to if statements

if shop_choice == "a"
    system("clear")
    puts "What would you like to buy?"
    menu.each do |item, cost| #lists items by item(name) and cost
        puts "#{item} ---------------- #{cost} gold"
    end 
        buy_choice = gets.chomp
        if buy_choice == "a"
            puts "sword aquired!"
            player["gold"] -= 100 #subtracts from gold
            player["atk"] += 1 #adds to attack
